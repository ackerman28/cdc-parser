...
# Split countries into high vs low PER-CAPITA ODA (median cutoff)
pc = df.groupby("Country")["oda_per_capita_calc"].first()
cutoff = pc.median()
high = set(pc[pc >= cutoff].index)
df["grp"] = np.where(df["Country"].isin(high), "High", "Low")
print(f"Per-capita ODA median cutoff: ${cutoff:.1f}/person")
print(f"High: {len(high)} countries | Low: {41 - len(high)} countries")
...
make_figure("High",
            f"Countries reporting each disease per month — High per-capita ODA ({len(high)} countries)",
            "analysis/fig_percapita_HIGH.png")

make_figure("Low",
            f"Countries reporting each disease per month — Low per-capita ODA ({41 - len(high)} countries)",
            "analysis/fig_percapita_LOW.png")